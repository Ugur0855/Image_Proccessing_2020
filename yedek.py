...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.perspectiveButton = QtWidgets.QPushButton(self.centralwidget)
        self.perspectiveButton.setMinimumSize(QtCore.QSize(75, 0))
        self.perspectiveButton.setObjectName("perpectiveButton")
        self.verticalLayout.addWidget(self.perspectiveButton)
        ...
        self.perspectiveButton.clicked.connect(self.perspectiveImage)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.perspectiveButton.setText(_translate("MainWindow", "Perspektif"))
...
# Added code here
    ...
    def perspectiveImage(self):
        if self.tmp is not None:
            M = cv2.matFromArray(2, 3, cv2.CV_64FC1, [1, 0, 50, 0, 1, 100])
            dst = cv2.Mat()
            dsize = cv2.Size(self.tmp.rows, self.tmp.cols)
            # You can try more different parameters
            cv2.warpAffine(self.tmp, dst, M, dsize, cv2.INTER_LINEAR, cv2.BORDER_CONSTANT, cv2.Scalar())
    ...
    def swirlImage(self):
        if self.tmp is not None:
            swirled = swirl(self.tmp, rotation=0, strength=10, radius=120)
            self.setPhoto(swirled)
